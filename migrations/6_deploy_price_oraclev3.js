...
module.exports = async function (deployer, network, accounts) {
  ...
  for (const tokenName in tokensUSD) {
    const priceUSD = await oracle.getPriceUSD(tokensUSD[tokenName]);
    console.log(`${tokenName} priceUSD: ${priceUSD.toString()}`);
  }

  for (const tokenName in tokensETH) {
    const priceETH = await oracle.getPriceETH(tokensETH[tokenName]);
